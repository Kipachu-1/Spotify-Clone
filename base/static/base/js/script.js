...
async function collection_content(uni_id, type) {
    ...
    for(let i=0; i <= data['musics'].length; i++) {
        ...
        // else {
        //     likedbtn.style.filter = 'invert(100%) sepia(7%) saturate(81%) hue-rotate(246deg) brightness(116%) contrast(100%)';
        // }
        music_name.addEventListener('click', async function (){
            window.history.pushState({}, '', `http://${host_address}/track/${data['musics'][i]['id']}`);
            clear_view_content();
...
async function get_last_history() {
    let data_last = await (await fetch(`http://${host_address}/history/get/last`)).json();
    if(data_last['last_track'] != 'no') {
    music_list = await (await fetch(`http://${host_address}/get/music/${data_last['last_track']}/info`)).json();
    sleep(0).then(()=>{
        loadTrack(track_index);
    });
}
